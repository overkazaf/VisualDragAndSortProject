	...
	Drag.prototype = {
		...
		initPosition : function () {
			...
			for(var i=0;i<obj.length;i++){
				if(obj.item(i).className=="area"){
					 obj_id.push(obj.item(i).id);
					}
			}
			
			var areaLen = obj_id.length;
			for (var j = 0; j < areaLen; j++ ) {
				var areaId = obj_id[j];
				//if(j==0) var areaId="hhhhhh"; else if(j==1) var areaId="fffsss"; else var areaId="bbbbbb"; 
		...
		initDragObj : function (e) {
			...
			if (this.overObj != target && target.id !='tmpbox') {
				...
				this.overObj.style.overflow  = 'hidden';
		...
		initMoveableArea : function () {
			...
			for (var i = 0; i < len; i++) {
				...
				el.ondblclick = function (e) {Drag.prototype.dragStart.call(_method, e);};
