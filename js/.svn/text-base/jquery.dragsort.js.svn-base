function DragSort(options) {
    ...
    var _default={
        ...
        helper: function () { return $('<div class="helper" style="height:20px; width:100px; background-color:red;color:Yellow; border:solid 1px gray;">拖拽此处</div>'); },
...
function DeleteWidget(id) {
    parent.DeleteWidget && parent.DeleteWidget(id);
}
...
function EditLayout(id) {
    ...
    //清空列
    $("#stylecol-part",doc).html("");
    childs.each(function(index){
        var widthCss = $(this).attr("style");
        var mat= widthCss.match(/(width\s*:\s*\S+\s*;*?)/ig);
        var unit = "";
        if(mat[0].indexOf("%") != -1){
            unit = "%";
        }else{
        ...
        var val = mat[0].match(/\d+/ig);
        var str = '<div class="stylerow-part">';
            str +='列'+(index+1);
            str +='<input type="text" class="col-w" value="'+val+'" /><i class="i-unit">'+unit+'</i>';
            str +='<br>边距设置(px)：';
            str +='<br>上<input type="text" class="col-mg mt" />';
            str +='下<input type="text" class="col-mg mb" />';
            str +='左<input type="text" class="col-mg ml" />';
            str +='右<input type="text" class="col-mg mr" />';
            str +='</div>';
        $("#stylecol-part",doc).append(str);
        if(index == 0){
            if(unit == "%"){
                $("#ps_unit",doc).click();
            }else{
    ...
    $("#bj_editor",doc).attr("relateId",id);
...
function CreateLoading() {
      var p = $('<img id="vdloading" src="../cmskj/img/ajax-loader.gif"/>');
    p.appendTo("body");
    return p;
}
