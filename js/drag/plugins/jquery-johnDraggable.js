...
(function($) {
    $.fn.johnDraggable = function(options) {
        ...
        var getPlaceHolderInstance = getInstance(function() {
            var emptyDiv = $('div[data-emptydiv=true]');
            if (!emptyDiv.length) {
              var emptyDiv = $('<div>').attr('data-emptydiv', true);
              emptyDiv.appendTo(wrapper);
            }
            return emptyDiv;
        });

        var hooksFixer = function($el, hooks) {
            if (!(typeof hooks == undefined)) {
                if ('list' == hooks || 'cont' == hooks) {
                    $el.attr('data-hooks', hooks);
                }
            }
        };
            ...
            var makeExternalRequire = function(extLib) {
                if (extLib) {
                    var importLibs = null,
                        jsArr = [],
                        cssArr = []
                    judgeFileType = function(filename, type) {
                        var
                            lFileName = filename.length,
                            lType = type.length;
                        return filename.lastIndexOf(type) === (lFileName - lType);
                    };
                    ...
                    $.each(importLibs, function() {
                        var source = $.trim(this.toString());
                        var l = source.length;
                        if (judgeFileType(source, 'js')) {
                            jsArr.push(source);
                        } else if (judgeFileType(source, 'css')) {
                            cssArr.push(source);
                        }
                    });
            ...
            var Drag = {
                ...
                drag: function(ev) {
                    ...
                    if (preLayoutCellArray.length) {
                        ...
                        var
                            target = $(opts.targetClass).eq(tt),
                            aWidgets = target.children('[class*=widget]'),
                            sortArray = [],
                            sal;
                        ...
                        if (p != -1) {
                            ...
                            if ($(that).data("widget")) {
                                var data = $(that).data('widget'),
                                    ...
                                    hooks = $(that).attr('data-widget-type');
                                ...
                                $.ajax({
                                    ...
                                    success: function(json) {
                                        ...
                                        // Here fix an attribute to the dom, this hooked attribute will be used later
                                        hooksFixer($dom, hooks);
                                        // Check if the wrapper contains some styles/links/scripts

                                        makeExternalRequire(json.css);

                                        try {
                                            // see widgetEventProxy.js
                                            WEProxy.scan($dom);
                                        } catch (e) {
                                            log(e);
                                        }
