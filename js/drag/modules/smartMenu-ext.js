...
var getInstance = function(fn) {
	var obj;
	return function() {
		return obj || (obj = fn.apply(arguments));
	}
};

var getModalInstance = getInstance(function() {
	return $('#configModal');
});
...
var createWidgetPanel = {
	...
	'article' : function (elemId, self, targetIndex, callback){
		var html = '',
			wcd = null,
			data = null,
			widget = null,
...
function extraFn(fnType, elemId, self) {
	...
	if (width && height) {
		...
		if (width.indexOf('%') == -1 && height.indexOf('%') == -1) {
			...
			//fix title line-height
			if (titleFlag) {
				tE.find('[desc=title]').css('line-height', tE.height() + 'px');
				tE.find('a').css('line-height', tE.height() + 'px');
			}
		}
	} else {
		if (height) {
	...
	if (imgUrl) {
		if (fnType == 'upload' || fnType == 'panel' || fnType == 'navbar-list') {
			var elW, elH;
				elW = elem.width(),
				elH = elem.height();
			if (fnType == 'upload') {
					elW = width || elem.width(),
					elH = height || elem.height();
			}

			var bg = 'background:url(/' + imgUrl + ') 0 0 no-repeat;background-size:' + elem.width() + 'px ' + elem.height() + 'px !important;';
		...
		if (!bgiUrl) {
			...
			if (fnType == 'footer' || fnType == 'vote' || fnType == 'imgchunk' || fnType == 'chunk' || fnType == 'link-panel' || fnType == 'panel' || fnType == 'navbar-list' || fnType == 'date' || fnType == 'weather') {
