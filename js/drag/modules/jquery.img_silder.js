    ...
    var g = {  //公共方法， 外部可调用
      //初始化
		init: function () {
			...
			var pagesize=0; //页码
			var silderList = $('#slider_list',g.obj);
			var silderList_li = $('#slider_list li',g.obj);
			...
			if(opt.is_showTit){ //判断是否显示标题
				currHtml += "<div class='slider_desc' id='slider_desc'></div>";
			}
			img_size = silderList_li.size() ;//图片个数
			
			currHtml += "<ul class='slider_page' id='slider_page'>";//分页码代码注入
			...
			var silderPage = $('#slider_page',g.obj);
			var silderPage_li =$('#slider_page li',g.obj);
			silderPage_li.eq(0).addClass('current');

			if(opt.is_showTit){ //初始化图片描述
				$('#slider_desc').text(silderList_li.eq(0).find('img').attr('alt'));
			}
			silderPage_li.on('click',function(){
				...
				if(opt.is_showTit){
					$('#slider_desc').text(silderList_li.eq(pagesize).find('img').attr('alt'));
				}
			});
